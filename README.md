# 👋 Hi, I'm 4Dcookie

## About Me
- 전공: 컴퓨터공학
- 관심 분야: 웹 개발, 데이터 분석, 게임 등
- 사용 가능한 기술 스택:
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=for-the-badge&logo=css3&logoColor=white)

![My Skills](https://skillicons.dev/icons?i=python,js,css,git,github)
- 이메일: wjdxodud0319@gmail.com

## 📌 진행 중인 프로젝트
### Korea Population Visualization
- 대한민국 인구 데이터를 다양한 그래프로 제공하는 웹 서비스
- GitHub 링크: https://github.com/Scanning-PKNU/korea-population
- 목표: 데이터 시각화로 인구 변화 통계 이해를 돕기

## 🧑‍💻 역할 & 기술
- 역할 : 커미터
- 데이터 수집 자동화, CSV 파싱 및 API 제공, 시각화 데이터 구조 설계를 담당함 (Python)
- Pull Requests:
  - PR #1, #2, #3 데이터 추출 코드 작성 및 수정
  - PR #20, #25, #35 GitHub 리포지토리 정리 & Readme 수정
  - PR #36, #45 Fetch API로 CSV 데이터를 로드해서 시각화하는 웹 코드 작성

## 🏆 활동 내역
| 유형 | 내용 | 링크 |
|------|------|------|
| **Pull Request** | 인구 데이터 시각화 기능 추가 | [PR #36](https://github.com/Scanning-PKNU/korea-population/pull/36) |
| **Pull Request** | CSV 데이터 추출 코드 개선 | [PR #1](https://github.com/Scanning-PKNU/korea-population/pull/1) |
| **Issue** | 그래프 미출력 버그 보고 및 해결 제안 | [Issue #12](https://github.com/Scanning-PKNU/korea-population/issues/12) |

![GitHub Contribution Graph](https://github-readme-activity-graph.vercel.app/graph?username=4Dcookie&theme=react-dark)

